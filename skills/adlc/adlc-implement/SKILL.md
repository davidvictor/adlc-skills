...
3. If executing a workstream step, read `WORKSTREAM.md`, `kanban.md`, the relevant milestone card, and the selected step card before editing.
4. For goal-managed workstreams, check the active Codex goal at the start and preserve the goal objective through closeout.
5. Read configured skill-context before fallback patch history.
6. Use `implement-coordinator` when native Codex agents are available.
7. Keep tightly coupled work local in the coordinator.
8. Delegate independent bounded edits to `implement-worker` with disjoint write scopes.
9. Run read-only sidecars after meaningful code changes: review, security, rules, docs, best-practices, and commit-preparer as relevant.
10. Integrate worker results, rerun verification, and update plan, milestone, or workstream step status.
